...
/**
 * Stamped by the `build` script. The `NEXT_PUBLIC_` prefix is what makes Next
 * inline the value at build time, so an ISR route regenerating a week later
 * still reports when the deployment was built rather than when that page was
 * rebuilt. Declaring it through `env` in `next.config.ts` -- the obvious
 * alternative -- makes Turbopack resolve every other server-side
 * `process.env` read to `undefined`.
 */
...
/** The stack in npm spec form: `["next@16.2.9", ...]`. */
export function getStack(): string[] {
  return STACK_DEPENDENCIES.flatMap((dependency) => {
    // Drops the range prefix, so `^4.3.1` reads as a version.
